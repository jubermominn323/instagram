...
const CreatePost = () =>{
        ...
        if (url){
            ...
            .then(data =>{
                if(data.error){
                    ...
                    M.toast({html: "Created Post Successfully", classes:"#43a047 green darken"})
                    history.push("/")
                }
            }).catch(err=>{
                console.log(err)
    ...
    const postDetails = () =>{
        ...
        data.append("cloud_name", "juber-insta")
        fetch("	https://api.cloudinary.com/v1_1/juber-insta/image/upload",{
            method: "post",
            body:data
        })
        .then(res => res.json())
